...
# JBDS_INSTALLERS :: CSV list (with spaces!) of additional installer jars to use. Will also use list of installers in JBDS_INSTALLERS_LISTFILE = install.devstudio.list.txt. 
...
# JBDS_INSTALLERS_LISTFILE :: used to find install.devstudio.list.txt; use either present working directory or passed in commandline arg
JBDS_INSTALLERS_LISTFILE=`pwd`/install.devstudio.list.txt
...
usage() {
  ...
  echo "  [ -JBDS_INSTALLERS_LISTFILE /path/to/install.devstudio.list.txt ]"
  ...
  echo ""
  echo "Example:"
  echo "  ./install.devstudio.sh -JAVA /opt/jdk1.8.0/bin/java -INSTALL_FOLDER /tmp/versionwatch-installations -JBDS_INSTALLERS_LISTFILE /dev/null \\"
...
while [[ "$#" -gt 0 ]]; do
  case $1 in
    ...
    '-JBDS_INSTALLERS_LISTFILE') JBDS_INSTALLERS_LISTFILE="$2"; shift 1;; # path to install.devstudio.list.txt or other file with CSV or one-per-line list of JBDS installers to run
...
if [[ ${JBDS_INSTALLER_NIGHTLY_FOLDER} ]] && [[ -d ${JBDS_INSTALLER_NIGHTLY_FOLDER} ]]; then 
  ...
  for i in `find ${JBDS_INSTALLER_NIGHTLY_FOLDER} -name "jboss-devstudio-*-installer-standalone.jar" -a -not -name "*latest*"`; do
    ...
    if [[ -d ${INSTALL_FOLDER}/devstudio-${f} ]] && [[ -f ${LATEST} ]] && [[ `cat ${LATEST}` == $ver ]]; then 
      echo "Existing JBDS install in ${INSTALL_FOLDER}/devstudio-${f} (${ver})"
    else
      # wipe existing installation
      if [[ ${f} ]] && [[ -d ${INSTALL_FOLDER}/devstudio-${f} ]]; then rm -fr ${INSTALL_FOLDER}/devstudio-${f}; fi
      # echo "Install JBDS ${f} (${ver}) to ${INSTALL_FOLDER}/devstudio-${f} ..."
      installJBDS ${f} ${i}
      echo "${ver}" > ${LATEST}
    fi
  done
...
for i in ${INSTALLER_LIST}; do
  ...
  if [[ -d ${INSTALL_FOLDER}/devstudio-${ver} ]]; then 
    echo "Existing JBDS install in ${INSTALL_FOLDER}/devstudio-${ver}"
  else
    # echo "Install JBDS ${ver} to ${INSTALL_FOLDER}/devstudio-${ver} ..."
    installJBDS ${ver} ${i%%,}
  fi
done
